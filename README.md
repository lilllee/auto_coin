...
- **인증**: 기본은 로그인 없이 바로 진입. 필요 시 `ENABLE_AUTH=1`로 password + TOTP 2FA 활성화 가능.
- **보안**: API 키 Fernet 암호화(`~/.auto_coin_master.key`, 0600). 인증 활성 시 세션 쿠키 7일 + 5회 실패 시 10분 lockout + 복구 코드 8개.
...
웹 UI 기본값은 **로그인 없이 바로 `/` 진입**입니다. 기존 `/setup` · `/login` · TOTP 보호 흐름이 필요하면
`.env`에 `ENABLE_AUTH=1`을 추가하세요.
...
### 4.1 기본값 — 로그인 없이 바로 진입
- 브라우저에서 `/` 접속 시 바로 대시보드 진입
- 하단 탭과 설정 화면을 즉시 사용 가능
- 인증 관련 페이지(`/login`, `/setup`, `/recovery`)는 기본적으로 `/`로 리다이렉트

### 4.2 선택 — `ENABLE_AUTH=1`로 보호 모드 켜기
`.env`에 아래를 추가한 뒤 웹 프로세스를 다시 시작하면 기존 인증 흐름이 활성화됩니다.
...
### 4.4 보호 모드의 다음 방문 (`/login`)
- 패스워드 + 현재 TOTP 6자리
- 세션 쿠키 7일 유지
- **5회 실패 → 10분 lockout**

### 4.5 보호 모드에서 TOTP 기기 상실 시
...
4. 필요 시 `ENABLE_AUTH=1` 설정 후 로그인 + TOTP

### 보안 체크
- 기본 무인증 진입이므로, 원격 접근 환경에서는 가능하면 `ENABLE_AUTH=1` 사용 권장
- 보호 모드 사용 시 V2 자체 인증(`/setup` 완료)
...
- `.env`가 있으면 자동으로 주입
- 기본값은 즉시 진입, 보호 모드가 필요하면 `.env`에 `ENABLE_AUTH=1`
...
| `~/.auto_coin_session.key` | 세션 쿠키 서명 (보호 모드 사용 시) | ⚠️ 유출 시 세션 탈취 가능 |
...
**초기화**: 세 파일 제거 → 보호 모드 사용 시 다시 `/setup`부터.
...
### `/login`이 바로 `/`로 돌아감
→ 현재 기본값은 무인증 진입. 보호 모드가 필요하면 `.env`에 `ENABLE_AUTH=1` 추가 후 재시작.

### 보호 모드에서 `/login`에 "사용자가 존재하지 않습니다"
